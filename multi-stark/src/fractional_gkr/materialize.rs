...
impl<F: Field> LookupPlan<F> {
    /// Materialize the packed numerator and denominator tables consumed by LogUp GKR.
    ///
    /// Every atomic contribution owns one exact-height block at
    /// `base_offset + (contribution_index << num_variables)`. The remaining tail is
    /// padded with the neutral fraction `0 / 1`.
    pub(crate) fn materialize_fraction<EF>(
        &self,
        main: &[&Table<F>],
