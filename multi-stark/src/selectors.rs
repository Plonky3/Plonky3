...
impl<EF: Field> BoundaryEvals<EF> {
    ...
    /// bound-coordinate prefix, spread across the lanes of `P` — one lane per
    /// consecutive residual row starting at `row`.
    ///
    ...
    /// bound-coordinate prefix — the twin of [`Self::row_pair_with_prefix`] over a
    /// packing of the same field.
    ///
    /// # Arguments
    ///
