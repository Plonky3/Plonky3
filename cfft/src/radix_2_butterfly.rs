...
impl<F: ComplexExtendable> CircleSubgroupFt<F> for Radix2Cft {
    type Evaluations = RowMajorMatrix<F>;

    fn cfft_batch(&self, mut mat: RowMajorMatrix<F>) -> <Self as CircleSubgroupFt<F>>::Evaluations {
        let n = mat.height();
        let log_n: usize = log2_strict_usize(n);

        let width = mat.width();

        let twiddles = cfft_twiddles::<F>(log_n, true); // These should be precomputed.
    ...
    fn coset_icfft_batch(
        ...
        let twiddles = coset_eval_twiddles::<F>(log_n, coset_elem); // Likely fast to precompute these.
...
pub fn cfft_twiddles<F: ComplexExtendable>(log_n: usize, inv: bool) -> Vec<Vec<F>> {
    let size = 1 << (log_n - 1);
    let init_domain = cfft_domain::<F>(log_n, size); // Get the starting domain.
...
pub fn coset_eval_twiddles<F: ComplexExtendable>(
    log_n: usize,
    coset_elem: Complex<F>,
) -> Vec<Vec<F>> {
    let size = 1 << (log_n - 1);
    let generator = F::circle_two_adic_generator(log_n - 1);

    let init_domain = twin_coset_domain::<F>(generator, coset_elem, size);
...
mod tests {
    ...
    #[test]
    fn fft_size_16() {
        fft_test::<Mersenne31, Radix2Cft, 16>();
    }

    #[test]
    fn fft_size_32() {
        fft_test::<Mersenne31, Radix2Cft, 32>();
    }

    #[test]
    fn ifft_size_16() {
        ifft_test::<Mersenne31, Radix2Cft, 16>();
    }

    #[test]
    fn ifft_size_32() {
        ifft_test::<Mersenne31, Radix2Cft, 32>();
    }

    #[test]
    fn fft_ifft_size_16_5() {
        fft_ifft_test::<Mersenne31, Radix2Cft, 16, 5>();
    }

    #[test]
    fn fft_ifft_size_32_3() {
        fft_ifft_test::<Mersenne31, Radix2Cft, 32, 3>();
    }

    #[test]
    fn coset_eval_size_16() {
        coset_eval_test::<Mersenne31, Radix2Cft, 16>();
    }

    #[test]
    fn coset_eval_size_32() {
        coset_eval_test::<Mersenne31, Radix2Cft, 32>();
    }

    #[test]
    fn coset_lde_size_16() {
        coset_lde_test::<Mersenne31, Radix2Cft, 16, 5>();
    }

    #[test]
    fn coset_lde_size_32() {
        coset_lde_test::<Mersenne31, Radix2Cft, 32, 3>();
    }
}
