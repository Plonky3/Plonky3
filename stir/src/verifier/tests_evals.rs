...
use crate::prover_eval::{commit_polynomial, prove, prove_round, StirRoundWitness};
...
fn tamper_with_final_polynomial(
    ...
    let domain_k_0 = witness
        .domain
        .shrink_coset(config.log_starting_folding_factor())
        .unwrap();

    let mut witness = StirRoundWitness {
        domain_l: witness.domain,
        domain_k: domain_k_0,
        evals_k: witness.evals_k, // NP TODO consistency of names; and/or use ..witness
    ...
    let final_polynomial = rand_poly_seeded(
        witness.evals_k.len() / 2_usize.pow(log_last_folding_factor as u32),
        Some(2512202),
    );
...
#[test]
// NP TODO remove or clean
fn test_bb_verify_compare() {
    ...
    let mut eval_prover_challenger = test_bb_challenger();
    let mut coeff_prover_challenger = eval_prover_challenger.clone();
    let mut eval_verifier_challenger = eval_prover_challenger.clone();
    let mut coeff_verifier_challenger = eval_prover_challenger.clone();
    let polynomial = rand_poly_seeded((1 << config.log_starting_degree()) - 1, Some(238567));

    let (eval_prover_witness, eval_prover_commitment) =
        commit_polynomial(&config, polynomial.clone());

    let start = Instant::now();
    let eval_prover_proof = prove(
        &config,
        eval_prover_witness,
        eval_prover_commitment,
        &mut eval_prover_challenger,
    );
    let duration = start.elapsed();
    println!("Time taken by eval-based prover: {:?}", duration);

    let (coeff_prover_witness, coeff_prover_commitment) =
        crate::prover::commit(&config, polynomial);

    let start = Instant::now();
    let coeff_prover_proof = crate::prover::prove(
        &config,
        coeff_prover_witness,
        coeff_prover_commitment,
        &mut coeff_prover_challenger,
    );
    let duration = start.elapsed();
    println!("Time taken by coeff-based prover: {:?}", duration);

    verify(
        &config,
        eval_prover_commitment,
        eval_prover_proof,
        &mut eval_verifier_challenger,
    )
    .unwrap();
    verify(
        &config,
        coeff_prover_commitment,
        coeff_prover_proof,
        &mut coeff_verifier_challenger,
    )
    .unwrap();

    // Check that the sponge is consistent at the end
    // NP TODO if sth like this is left in, make sure zeros are trimmed in the new prover
    assert_eq!(
        eval_prover_challenger.sample_algebra_element::<BbExt>(),
        coeff_prover_challenger.sample_algebra_element::<BbExt>()
    );
}
