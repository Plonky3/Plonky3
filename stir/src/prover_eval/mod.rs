...
// What I was suggesting was slightly different. My though was to try and avoid
// all the divisions involved in computing quotient_polynomial and the
// multiplication in multiply_by_power_polynomial.

// My mental model for something you could try is:

// 1. Take as input the evaluations of f_{i - 1} over a sub-coset  K_{i - 1} of
// L_{i - 1} (We don't need all the evaluations as we know that the degree of
// f_{i - 1} is much lower than |L_{i - 1}|. Also by sub-coset I mean if L_{i -
// 1} = gH then a sub coset would be K_{i - 1} = gH' for H' a subgroup of H.)

// 2. Fold(evals of f_{i - 1} over K_{i - 1}, k_{i - 1}) in order to obtain the
//    evals of g_i over (K_{i - 1})^(k_{i - 1}).

// 3. Use an IFFT to get the coefficients of g_i and then another FFT to get the
// evaluations of g_i over L_i.

// 4. Use the saved coefficients of g_i to compute the betas and
// stir_randomness_evals.

// 5. Pick a small sub-coset K_i of L_i such that g_i has degree equal to |L_i| -
// 1.

// 6. Compute the evaluations of the answer (computed in coefficient form using
//    Lagrange interpolation), vanishing (computed in coefficient form using the
//    definition), and power polynomials (computed in coefficient form using the
//    definition) on K_i.

// 7. Use a batch inverse to invert the evaluations vector of the vanishing
// polynomials.

// 8. Compute f_i = power * (g_i - answer)/quotient and output this for the next
// step.

// Would that work or am I missing something?

// Assuming this works, it lets us avoid the costly polynomial division and
// multiplications. For the sum and products of evaluations vectors we can also
// use PackedField elements when possible, which makes these sorts of operations
// much faster.

// The drawback is needing an IFFT initially to get coefficients of g_i and the
// cost of computing the evaluations of the answer, vanishing, power polynomials
// on K_i. My sense is that this is a worthwhile tradeoff though as K_i should
// get small fast and so the FFT's would be cheap.
