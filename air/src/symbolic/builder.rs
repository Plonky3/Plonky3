...
impl<F: Field, EF: ExtensionField<F>> SymbolicAirBuilder<F, EF> {
    /// Create a new `SymbolicAirBuilder` without periodic columns.
    pub fn new(
        preprocessed_width: usize,
        width: usize,
        num_public_values: usize,
    ...
    /// Create a new `SymbolicAirBuilder` with periodic columns.
...
mod tests {
    ...
    fn test_symbolic_air_builder_with_periodic() {
        let builder = SymbolicAirBuilder::<BabyBear>::new_with_periodic(2, 4, 3, 0, 0, 3);

        let periodic = builder.periodic_values();
        assert_eq!(periodic.len(), 3, "Should have 3 periodic columns");

        for (i, var) in periodic.iter().enumerate() {
            assert_eq!(var.entry, Entry::Periodic);
