...
mod tests {
    ...
    fn test_hash_challenger() {
        let initial_state = (1..11_u8).map(|i| i.into()).collect::<Vec<_>>();
        ...
        let expected_sum = 55_u8.into();
        let expected_len = 10_u8.into();
        assert_eq!(
        ...
        assert_eq!(new_element, new_expected_len.into());
        assert_eq!(hash_challenger.output_buffer, [new_expected_sum.into()])
    }
}
