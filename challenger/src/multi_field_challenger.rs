...
use crate::{CanObserve, CanSample, CanSampleBits, DuplexChallenger, FieldChallenger};
...
mod tests {
    ...
    fn test_finalize() {
        ...
        // Finalize: flush pending F values so the absorb actually lands in the sponge state.
        c1.flush_f_if_non_empty();
        c2.flush_f_if_non_empty();
        let c1_digest = &c1.inner.sponge_state[..RATE];
        let c2_digest = &c2.inner.sponge_state[..RATE];
        assert_eq!(c1_digest, c2_digest);
    ...
    fn test_finalize_sample_interaction() {
        ...
        let digest = |n_samples: usize| {
            ...
            c.inner.duplexing();
            let d: [PF; RATE] = c.inner.sponge_state[..RATE].try_into().unwrap();
            d
        };
    ...
    fn test_partial_absorb_length_distinct_from_padded_equivalent() {
        ...
        // Finalize: flush pending F values so the absorb actually lands in the sponge state.
        a.flush_f_if_non_empty();
        b.flush_f_if_non_empty();
        let a_digest = &a.inner.sponge_state[..RATE];
        let b_digest = &b.inner.sponge_state[..RATE];
    ...
    fn test_observe_hash_native_pf_high_bits_distinct() {
        ...
        let digest = |h: PF254| {
            ...
            c.inner.duplexing();
            let d: [PF254; RATE] = c.inner.sponge_state[..RATE].try_into().unwrap();
            d
        };
    ...
    fn test_observe_hash_native_vs_expanded_f_not_equal() {
        ...
        // Finalize
        native.inner.duplexing();
        via_f.inner.duplexing();
        let native_digest = &native.inner.sponge_state[..RATE];
        let via_f_digest = &via_f.inner.sponge_state[..RATE];
        assert_ne!(native_digest, via_f_digest);
    }
