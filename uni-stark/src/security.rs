...
pub enum OpeningShape {
    /// Ordinary two-adic FRI openings.
    #[default]
    TwoAdic,
    /// Circle's DEEP quotient consumes two batching powers per column and opening point.
    Circle,
    /// Hiding FRI adds a randomizing codeword, increases the quotient degree,
    /// and appends random columns to every non-preprocessed matrix.
    Hiding {
        /// The `num_random_codewords` passed to `HidingFriPcs::new`.
...
impl OpeningShape {
    ...
    /// Hiding FRI with the same random-column count as `HidingFriPcs::new`.
    pub const fn hiding(num_random_codewords: usize) -> Self {
        Self::Hiding {
            num_random_codewords,
        }
    }

    /// Whether the PCS randomizes the trace and quotient domains.
    pub const fn is_zk(self) -> bool {
        matches!(self, Self::Hiding { .. })
    }

    /// Additional base-field columns on each non-preprocessed matrix.
    pub const fn num_random_codewords(self) -> usize {
        match self {
            Self::TwoAdic | Self::Circle => 0,
            Self::Hiding {
                num_random_codewords,
            } => num_random_codewords,
        }
    ...
    /// Powers of the batching challenge consumed per column and opening point.
    pub const fn powers_per_opening(self) -> usize {
        match self {
            Self::Circle => 2,
            Self::TwoAdic | Self::Hiding { .. } => 1,
        }
    }
}

/// Number of batching powers consumed by the polynomial commitment scheme's
/// random linear combination into its single low-degree-test instance.
///
...
/// - hiding FRI's additional random columns on the main trace, quotient chunks,
///   and randomizing codeword. Preprocessing adds no random columns.
///
/// This is the `k` of the batched-openings proximity term, whose error grows
/// with `log2(k − 1)`. Circle uses two powers per `(column, opening point)` pair,
/// so [`OpeningShape::Circle`] doubles the count above.
pub const fn num_batched_openings(
    ...
    let random_columns = openings.num_random_codewords();
    let main = if main_next { 2 } else { 1 } * (main_width + random_columns);
    let preprocessed = if preprocessed_next { 2 } else { 1 } * preprocessed_width;
    let quotient = num_quotient_chunks * (challenge_dimension + random_columns);
    let random = if openings.is_zk() {
        challenge_dimension + random_columns
    } else {
        0
    };
    (main + preprocessed + quotient + random) * openings.powers_per_opening()
}
...
pub struct StarkSecurityParams {
    ...
    pub max_combo: usize,
    /// Number of powers in the polynomial commitment scheme's batching combination:
    /// trace columns, preprocessed columns, quotient chunks, and the zk
    /// randomizing codeword. Circle consumes two powers per `(column, opening point)` pair.
    ///
...
mod tests {
    ...
    fn num_batched_openings_counts_every_committed_column_and_point() {
        ...
        assert_eq!(
            num_batched_openings(100, true, 0, false, 2, 4, OpeningShape::new()),
            208
        );
        // Without next-row access the trace is opened at `zeta` alone.
        assert_eq!(
            num_batched_openings(100, false, 0, false, 2, 4, OpeningShape::new()),
            108
        );
        // Preprocessed columns follow the same rule.
        assert_eq!(
            num_batched_openings(100, true, 6, true, 2, 4, OpeningShape::new()),
            220
        );
        // Zero-knowledge adds one randomizing codeword.
        assert_eq!(
            num_batched_openings(100, true, 0, false, 2, 4, OpeningShape::hiding(0)),
            212
        );
    }
