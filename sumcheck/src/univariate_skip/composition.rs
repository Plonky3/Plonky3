...
///     in the residual rounds     ->  large-field values, after round one
/// ```
///
/// One implementation covers both: only the carrier changes.
/// A trait rather than a closure lets the round read it in either carrier.
///
/// # Degree
///
...
///
/// The verifier imposes vanishing and reconstructs below the size sent.
/// A broken witness therefore still leaves a difference, and is caught.
/// What breaks is that honest proofs stop fitting, and are rejected.
///
/// Declaring it too high costs cosets.
/// It also widens the domain reconstructed on, which the budget charges for.
...
/// # Overview
///
/// Three operands, the third the bitwise conjunction of the first two:
///
/// ```text
///     a & b = c        on bits,  a * b = c
/// ```
...
impl<F> Composition<F> for Conjunction {
    ...
    fn eval<A: Algebra<F> + Copy>(&self, values: &[A]) -> A {
        ...
        // Written as a difference, so it means the same in any characteristic.
...
/// # Overview
///
/// Three operands, the third the square of the first times the second:
///
/// ```text
///     a^2 * b = c
/// ```
///
/// Total degree three, so it needs one transmitted dimension more.
...
mod tests {
    ...
    /// Read the constraint in the alphabet itself, as transmitted points are.
    ...
    fn one_constraint_reads_in_both_carriers() {
        // The round reads the same constraint over subfield and large values.
        ...
        //
        // The operands are not bits: transmitted points carry extended rows.
        // The product there is a subfield multiplication rather than an and.
    ...
    fn the_cubic_constraint_vanishes_exactly_where_it_should() {
        // Fixture state: random subfield triples, third operand satisfying.
        //
    ...
    fn the_declared_shape_matches_what_the_constraint_reads() {
        // The arity sizes the tuple, the degree the transmitted domain.
        // Both are read by the round rather than inferred, so they are pinned.
