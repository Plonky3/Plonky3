...
use p3_binary_field::{BinaryField64, BinaryField128, Ghash128, Poly64, Poly192};
...
/// The cubic extension over its coefficient field, each entry at the constant limb.
impl FromTable<Poly64> for Poly192 {}
        ...
        for order in [VariableOrder::Prefix, VariableOrder::Suffix] {
            ...
            for split in 0..3 {
                ...
                for &rounds in &batches[..split] {
                    // The tail replays these challenges, so the head only advances the sponge.
