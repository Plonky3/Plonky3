...
use p3_fri::{FriConfig, FriConfigImpl, FriLdt};
use p3_ldt::Ldt;
...
type MyFriConfig = FriConfigImpl<Val, Domain, Challenge, ChallengeMmcs, ChallengeMmcs, Challenger>;

fn get_ldt_for_testing() -> (Perm, ChallengeMmcs, FriLdt<MyFriConfig>) {
...
fn test_fri_ldt() {
    ...
    let ldes: Vec<RowMajorMatrix<Challenge>> = (3..4)
        .map(|deg_bits| {
