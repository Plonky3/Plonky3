...
mod wide {
    ...
    use crate::packed::{HIGH_BY_HIGH, HIGH_BY_LOW, LOW_BY_HIGH, LOW_BY_LOW, PackedGhash128};
    ...
    impl SplitScalar {
        ...
        #[inline]
        fn new(scalar: u128) -> Self {
            let t = lanes::broadcast(scalar);

            // The companion is one Horner step with nothing underneath the shifted part.
            let t_x64 = PackedGhash128::fold_shifted(lanes::zero(), t);
        ...
        #[inline]
        fn apply(self, v: lanes::Reg) -> lanes::Reg {
            // Limbs 0 and 1: the half products that carry no further shift.
            let low = lanes::xor(
                lanes::clmul::<LOW_BY_LOW>(v, self.t),
                lanes::clmul::<HIGH_BY_LOW>(v, self.t_x64),
            );

            // Limbs 1 and 2: the half products weighted by `x^64`.
            let high = lanes::xor(
                lanes::clmul::<LOW_BY_HIGH>(v, self.t),
                lanes::clmul::<HIGH_BY_HIGH>(v, self.t_x64),
            );

            // Three limbs, so one Horner step in `x^64` finishes the reduction.
            PackedGhash128::fold_shifted(low, high)
        }
    }
