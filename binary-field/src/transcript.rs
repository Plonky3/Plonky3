...
//!
//! The narrow levels hold one byte or two, so that length field spans several elements.
//! Writing it at a fixed eight bytes keeps one rule for every level.
    ...
    ($name:ty, $repr:ty, $bits:literal) => {
        impl TranscriptField for $name {
            ...
            fn observe_seed<C: CanObserve<Self>>(challenger: &mut C, bytes: &[u8]) {
                ...
                // The length goes first, at a width that does not depend on the level.
                //
                // A narrow level holds only part of it, so it spans several elements there.
