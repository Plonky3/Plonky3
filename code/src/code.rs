...
pub trait CodeOrFamily<F: Field, In: MatrixRows<F>> {
    type Out: for<'a> MatrixRows<F>;

    fn encode_batch(&self, messages: In) -> Self::Out;
}

/// A code (in the coding theory sense).
pub trait Code<F: Field, In: for<'a> MatrixRows<F>>: CodeOrFamily<F, In> {
...
pub trait CodeFamily<F: Field, In: for<'a> MatrixRows<F>>: CodeOrFamily<F, In> {
...
/// A linear code.
pub trait LinearCode<F: Field, In: for<'a> MatrixRows<F>>: Code<F, In> {}

/// A family of linear codes.
pub trait LinearCodeFamily<F: Field, In: for<'a> MatrixRows<F>>: CodeFamily<F, In> {}
