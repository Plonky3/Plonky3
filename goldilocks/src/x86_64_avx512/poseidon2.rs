...
/// Apply the Poseidon2 S-box (`x^7`) to every element of a packed AVX512 state.
///
...
fn sbox_array<const WIDTH: usize>(state: &mut [PackedGoldilocksAVX512; WIDTH]) {
    ...
    let x3: [PackedGoldilocksAVX512; WIDTH] = core::array::from_fn(|i| x2[i] * state[i]);
    let x4: [PackedGoldilocksAVX512; WIDTH] = core::array::from_fn(|i| x2[i].square());
    for i in 0..WIDTH {
        state[i] = x3[i] * x4[i];
    }
}
...
/// light permutation) to a packed AVX512 state.
#[inline(always)]
