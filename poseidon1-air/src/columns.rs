...
> {
    ...
    /// resistance against statistical attacks (see Poseidon1 paper, Section 5.5.1).
...
///
/// Uses `size_of::<Poseidon1Cols<u8, ...>>()` since `u8` has size 1.
...
pub const fn num_cols<
    ...
    const PARTIAL_ROUNDS: usize,
>() -> usize {
    size_of::<Poseidon1Cols<u8, WIDTH, SBOX_DEGREE, SBOX_REGISTERS, HALF_FULL_ROUNDS, PARTIAL_ROUNDS>>(
    )
}
...
impl<
    ...
    const HALF_FULL_ROUNDS: usize,
    const PARTIAL_ROUNDS: usize,
> Borrow<Poseidon1Cols<T, WIDTH, SBOX_DEGREE, SBOX_REGISTERS, HALF_FULL_ROUNDS, PARTIAL_ROUNDS>>
    for [T]
{
    fn borrow(
        &self,
...
impl<
    ...
    const HALF_FULL_ROUNDS: usize,
    const PARTIAL_ROUNDS: usize,
> BorrowMut<Poseidon1Cols<T, WIDTH, SBOX_DEGREE, SBOX_REGISTERS, HALF_FULL_ROUNDS, PARTIAL_ROUNDS>>
    for [T]
{
    fn borrow_mut(
        &mut self,
    ) -> &mut Poseidon1Cols<T, WIDTH, SBOX_DEGREE, SBOX_REGISTERS, HALF_FULL_ROUNDS, PARTIAL_ROUNDS>
    {
        let (prefix, shorts, suffix) = unsafe {
            self.align_to_mut::<Poseidon1Cols<
                T,
                WIDTH,
                SBOX_DEGREE,
                SBOX_REGISTERS,
