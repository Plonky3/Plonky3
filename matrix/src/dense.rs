...
impl<T: Clone> RowMajorMatrix<T> {
        ...
        P: PackedValue<Value = T>,
    {
        ...
        (0..self.width).map(move |col| P::from_fn(|i| self.get(r + i, col).clone()))
    }
...
impl<T: Clone> Matrix<T> for RowMajorMatrix<T> {
    fn width(&self) -> usize {
        self.width
    }
    ...
    fn row(&self, r: usize) -> impl Iterator<Item = T> {
...
impl<T: Clone> Matrix<T> for RowMajorMatrixView<'_, T> {
    fn width(&self) -> usize {
        self.width
    }
...
impl<'a, T: Clone> RowMajorMatrixViewMut<'a, T> {
    #[must_use]
...
impl<T: Clone> Matrix<T> for RowMajorMatrixViewMut<'_, T> {
    fn width(&self) -> usize {
        self.width
    }
