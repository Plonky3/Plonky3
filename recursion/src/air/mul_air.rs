...
impl FieldOpAir {
        ...
        StandardUniform: Distribution<F>,
    {
        ...
        for (i, (left_addr, left_val, right_addr, right_val, res_addr, res_val)) in
            trace_values.iter_mut().tuples().enumerate()
        {
            (*left_addr, *left_val) = (F::from_usize(3 * i), rng.random());
            (*right_addr, *right_val) = (F::from_usize(3 * i + 1), rng.random());
            (*res_addr, *res_val) = (
                F::from_usize(3 * i + 2),
                self.op.apply(*left_val, *right_val),
            );
...
impl<F> BaseAir<F> for FieldOpAir {
    fn width(&self) -> usize {
        TRACE_WIDTH
    }
}

impl<AB: AirBuilder> Air<AB> for FieldOpAir {
    fn eval(&self, builder: &mut AB) {
        ...
        for i in 0..REPETITIONS {
            let start = i * 6;
            let left_val = main_local[start + 1].clone();
            let right_val = main_local[start + 3].clone();
            let res_val = main_local[start + 5].clone();
            builder.assert_zero(self.op.apply(left_val, right_val) - res_val);
        }
    }
}
