...
#[inline(always)]
fn diagonal_mul_32(state: &mut [PackedMersenne31AVX512; 32]) {
    state[2] = state[2] + state[2];
    state[3] = mul_2exp_i::<2, 29>(state[3]);
    state[4] = mul_2exp_i::<3, 28>(state[4]);
    state[5] = mul_2exp_i::<4, 27>(state[5]);
...
#[inline(always)]
fn internal_32(state: &mut [PackedMersenne31AVX512; 32], rc: __m512i) {
    state[0] = add_rc_and_sbox(state[0], rc);
    let sum_tail = PackedMersenne31AVX512::sum_array::<31>(&state[1..]);
    let sum = sum_tail + state[0];
    state[0] = sum_tail - state[0];
...
impl InternalLayer<PackedMersenne31AVX512, 32, 5> for Poseidon2InternalLayerMersenne31 {
    fn permute_state(&self, state: &mut [PackedMersenne31AVX512; 32]) {
        self.packed_internal_constants
            .iter()
