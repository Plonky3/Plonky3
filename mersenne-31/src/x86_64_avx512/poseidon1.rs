//! AVX512-optimized Poseidon1 permutation layers for Mersenne31.
//!
//! Provides [`Poseidon1ExternalLayerMersenne31`] (full rounds) and
//! [`Poseidon1InternalLayerMersenne31`] (partial rounds), implementing
//! [`FullRoundLayer`] and [`PartialRoundLayer`] for [`PackedMersenne31AVX512`].
//!
//! # Optimization Strategy
//!
//! **Full rounds** — round constants are pre-packed into `__m512i` AVX512
...
/// AVX512-optimized external (full-round) layer for Mersenne31 Poseidon1.
///
...
/// - **Packed** (`Vec<[__m512i; WIDTH]>`) — each constant broadcast to
///   all sixteen AVX512 lanes in negative form, enabling the fused [`add_rc_and_sbox`] path.
#[derive(Clone)]
    ...
    for Poseidon1ExternalLayerMersenne31<WIDTH>
{
    fn new_from_constants(constants: FullRoundConstants<Mersenne31, WIDTH>) -> Self {
        let pack_rc = |rcs: &[[Mersenne31; WIDTH]]| -> Vec<[__m512i; WIDTH]> {
            rcs.iter()
...
/// Apply a sequence of full rounds using AVX512-packed negative-form constants.
///
...
fn full_rounds_packed<const WIDTH: usize>(
    ...
    packed_constants: &[[__m512i; WIDTH]],
) where
    MdsMatrixMersenne31: Permutation<[PackedMersenne31AVX512; WIDTH]>,
{
    let mds = MdsMatrixMersenne31;
    for rc in packed_constants {
        for (s, &c) in state.iter_mut().zip(rc.iter()) {
            *s = add_rc_and_sbox(*s, c);
        }
        mds.permute_mut(state);
    }
}

/// Packed AVX512 path: fused AddRC + S-box with pre-packed negative-form constants.
impl<const WIDTH: usize> FullRoundLayer<PackedMersenne31AVX512, WIDTH, 5>
    for Poseidon1ExternalLayerMersenne31<WIDTH>
where
    MdsMatrixMersenne31: Permutation<[PackedMersenne31AVX512; WIDTH]>,
{
...
/// AVX512-optimized internal (partial-round) layer for Mersenne31 Poseidon1.
///
...
/// impl on `PackedMersenne31AVX512` handles broadcasting during multiplication.
///
...
/// Packed AVX512 path: S-box on `state[0]` only, sparse matmul via scalar constants.
impl<const WIDTH: usize> PartialRoundLayer<PackedMersenne31AVX512, WIDTH, 5>
    for Poseidon1InternalLayerMersenne31<WIDTH>
{
    fn permute_state(&self, state: &mut [PackedMersenne31AVX512; WIDTH]) {
        for (s, &rc) in state
            .iter_mut()
...
mod tests {
    ...
    /// Known-answer test for width 16 through the AVX512 packed path.
    #[test]
    fn test_avx512_poseidon1_width_16() {
    ...
    #[test]
    fn test_avx512_poseidon1_width_32() {
    ...
    proptest! {
        #[test]
        fn poseidon1_avx512_matches_scalar_width_16(
        ...
        #[test]
        fn poseidon1_avx512_matches_scalar_width_32(
