...
// `as_chunks` requires a standalone const-generic chunk size on stable; `N * D * 2` and
// `D * 2` aren't standalone, so `chunks_exact`/`chunks_exact_mut` stay the only option here.
