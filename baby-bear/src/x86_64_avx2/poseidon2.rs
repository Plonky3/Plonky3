...
use crate::{
    BabyBear, DiffusionMatrixBabybear, PackedBabyBearAVX2, POSEIDON2_INTERNAL_MATRIX_DIAG_16_BABYBEAR_MONTY,
...
impl Permutation<[PackedBabyBearAVX2; 16]> for DiffusionMatrixBabybear {
    fn permute_mut(&self, state: &mut [PackedBabyBearAVX2; 16]) {
        matmul_internal::<BabyBear, PackedBabyBearAVX2, 16>(state, POSEIDON2_INTERNAL_MATRIX_DIAG_16_BABYBEAR_MONTY);
...
impl Permutation<[PackedBabyBearAVX2; 24]> for DiffusionMatrixBabybear {
    fn permute_mut(&self, state: &mut [PackedBabyBearAVX2; 24]) {
        matmul_internal::<BabyBear, PackedBabyBearAVX2, 24>(state, POSEIDON2_INTERNAL_MATRIX_DIAG_24_BABYBEAR_MONTY);
        state.iter_mut().for_each(|i| *i *= MONTY_INVERSE);
    }
}
