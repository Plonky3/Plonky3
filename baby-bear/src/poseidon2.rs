...
fn matmul_internal<const WIDTH: usize>(
    ...
    let sum: BabyBear = state.iter().cloned().sum();
    for i in 0..WIDTH {
        state[i] *= BabyBear::from_canonical_u32(mat_internal_diag_m_1[i]);
        state[i] += sum.clone();
    }
}
...
impl Permutation<[BabyBear; 16]> for DiffusionMatrixBabybear {
    fn permute_mut(&self, state: &mut [BabyBear; 16]) {
        matmul_internal::<16>(state, MATRIX_DIAG_16_BABYBEAR);
    }
}
...
impl Permutation<[BabyBear; 24]> for DiffusionMatrixBabybear {
    fn permute_mut(&self, state: &mut [BabyBear; 24]) {
        matmul_internal::<24>(state, MATRIX_DIAG_24_BABYBEAR);
    }
}
...
mod tests {
    ...
    use super::{
        BabyBear, DiffusionMatrixBabybear
    };
