...
use super::generation::{Blake3CompressionInput, generate_binary_trace_rows};
...
impl Blake3BinaryAir {
    ...
    /// a block length of 64 and no flags.
    ///
...
impl<F> BaseAir<F> for Blake3BinaryAir {
    fn width(&self) -> usize {
        NUM_BLAKE3_BINARY_COLS
    }
...
///
/// The `b` and `d` words are always trace columns. The `a` and `c` words are either
/// initial values or the XOR of two trace columns.
...
impl<AB: AirBuilder> Air<AB> for Blake3BinaryAir {
    #[inline]
    fn eval(&self, builder: &mut AB) {
        let main = builder.main();
        let local: &Blake3BinaryCols<AB::Var> = main.current_slice().borrow();
...
/// The bits of `x ^ (y <<< shift)`.
fn xor_rotl<AB: AirBuilder>(x: &[AB::Var; 32], y: &[AB::Var; 32], shift: usize) -> [AB::Expr; 32] {
