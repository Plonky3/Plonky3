...
fn test_compute_sum_terms_logic() {
    ...
    let (num, den) = LogUpGadget::compute_combined_sum_terms::<MockAirBuilder, F, F>(
        &elements,
        &multiplicities,
        &alpha,
        &beta,
...
fn test_empty_lookup_is_valid() {
    ...
    let (num, den) =
        LogUpGadget::compute_combined_sum_terms::<MockAirBuilder, F, F>(&[], &[], &alpha, &alpha);
