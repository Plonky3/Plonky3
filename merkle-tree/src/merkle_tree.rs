    ...
    MerkleTree<F, W, M, DIGEST_ELEMS>
{
        ...
        W: Clone,
    {
        ...
        // Layer index from leaves: the root is at `num_layers - 1`.
        // For cap_height = 0, we want the root layer.
        // For cap_height = h, we want `num_layers - 1 - h`.
        let layer_idx = num_layers - 1 - cap_height;
        let layer = &self.digest_layers[layer_idx];

        // The layer may have some padding; we need exactly 2^cap_height elements.
    ...
    /// Returns the number of digest layers in the tree.
    #[must_use]
