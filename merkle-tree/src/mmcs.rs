...
impl<P, PW, H, C, const DIGEST_ELEMS: usize> MerkleTreeMmcs<P, PW, H, C, DIGEST_ELEMS> {
    ...
    /// Returns the configured cap height.
    ...
    [PW::Value; DIGEST_ELEMS]: Serialize + for<'de> Deserialize<'de>,
{
    ...
    fn commit<M: Matrix<P::Value>>(
        ...
        // Clamp cap_height to fit this tree (small trees during FRI folding may have fewer layers)
    ...
    fn verify_batch(
        ...
        // commit.len() == 2^effective_cap_height (clamped during commit for small trees).
...
mod tests {
    ...
    #[test]
    fn cap_height_clamped_for_small_trees() {
    ...
    fn single_row_matrix_with_cap_height() {
        ...
        // max cap_height = 1 - 1 = 0, so any cap_height > 0 gets clamped.
        let mmcs = MyMmcs::new(hash, compress, 5);
        let mat = RowMajorMatrix::<F>::rand(&mut rng, 1, 8);
        let (cap, prover_data) = mmcs.commit(vec![mat]);

        // Cap should have 1 entry (clamped to height 0 = root)
        assert_eq!(cap.len(), 1);
    ...
    fn cap_height_exact_boundary() {
        ...
        // Test cap_height = 4 (exact boundary, no clamping).
