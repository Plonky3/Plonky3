...
// One Keccak state per lane of a packed permutation.
type PackedMmcs<F, const N: usize> =
    MerkleTreeMmcs<[F; VECTOR_LEN], [u64; VECTOR_LEN], Hasher, Compression<N>, N, 4>;

// The same sponge, one message at a time: an independent reference for the packed arm.
type ScalarMmcs<F, const N: usize> = MerkleTreeMmcs<F, u64, Hasher, Compression<N>, N, 4>;
    ...
    StandardUniform: Distribution<F>,
{
    ...
    let packed = PackedMmcs::<F, N>::new(hasher, Compression::new(sponge), cap_height);
    let reference = ScalarMmcs::<F, N>::new(hasher, Compression::new(sponge), cap_height);
    let (cap, data) = packed.commit(matrices.clone());
    ...
    for index in [0, height / 2, height - 1] {
        let opening = packed.open_batch(index, &data);
...
fn packed_rows_preserve_salted_commitments() {
    type Hiding<P, PW> = MerkleTreeHidingMmcs<P, PW, Hasher, Compression<4>, StdRng, 4, 4, 5>;
    let mut rng = SmallRng::seed_from_u64(73);
    ...
    let packed = Hiding::<[BabyBear; VECTOR_LEN], [u64; VECTOR_LEN]>::new(
        hasher,
        Compression::new(sponge),
        1,
        StdRng::seed_from_u64(91),
    );
    let reference = Hiding::<BabyBear, u64>::new(
        hasher,
        Compression::new(sponge),
        1,
        StdRng::seed_from_u64(91),
