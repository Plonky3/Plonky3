...
//!
//! This crate provides [`Poseidon`], an optimized implementation that uses the **sparse matrix
...
//! # Round Structure
//!
//! Poseidon alternates between two types of rounds:
//!
//! ```text
//!   ┌──────────────────┐   ┌───────────────────┐   ┌──────────────────┐
...
impl<F: PrimeField, const WIDTH: usize> PoseidonConstants<F, WIDTH> {
    ...
    pub fn to_optimized(
        ...
        let (first_round_constants, opt_partial_rc, m_i, sparse_v, sparse_w_hat) =
            utils::compute_optimized_constants::<F, WIDTH>(&self.mds, rounds_p, &partial_rc);
...
pub struct Poseidon<F, FullRoundPerm, PartialRoundPerm, const WIDTH: usize, const D: u64> {
    ...
    /// The RP partial round layer using the sparse matrix decomposition.
    ///
    /// Each partial round:
