...
fn bench_poseidon(c: &mut Criterion) {
    // BabyBear width-16: textbook MDS-per-round is faster (Karatsuba on power-of-2 width).
    poseidon_textbook::<BabyBear, BabyBear, MdsMatrixBabyBear, 16, 7>(c);
    // BabyBear width-24+: sparse decomposition is faster.
...
/// Benchmark using the sparse matrix decomposition (O(t) per partial round).
...
/// Benchmark using textbook MDS-per-round with forward-substituted scalar constants.
/// Faster when the MDS permutation uses sub-O(t^2) convolution (e.g., Karatsuba).
fn poseidon_textbook<F, A, Mds, const WIDTH: usize, const ALPHA: u64>(c: &mut Criterion)
where
    F: PrimeField + InjectiveMonomial<ALPHA>,
    A: Algebra<F> + Sync + InjectiveMonomial<ALPHA>,
    StandardUniform: Distribution<F>,
    Mds: Permutation<[F; WIDTH]> + Permutation<[A; WIDTH]> + Default + Sync + Clone,
{
    ...
    let poseidon =
        Poseidon::<F, External<F, Mds, WIDTH>, Internal<F, Mds, WIDTH>, WIDTH, ALPHA>::new_from_rng(
            half_num_full_rounds,
            num_partial_rounds,
            &Mds::default(),
            &mut rng,
