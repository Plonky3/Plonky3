//! AVX2-optimized Poseidon1 layers for MontyField31.
...
use crate::{
    FieldParameters, MDSUtils, MontyField31, PackedMontyField31AVX2, PackedMontyParameters,
    PartialRoundBaseParameters, RelativelyPrimePower, apply_func_to_even_odd,
};

/// AVX2-specific trait for Poseidon1 partial round parameters.
pub trait PartialRoundParametersAVX2<PMP: PackedMontyParameters, const WIDTH: usize>:
    Clone + Sync
{
}

/// The internal (partial round) layer of Poseidon1 for AVX2-packed MontyField31.
#[derive(Clone)]
...
> {
    ...
    packed_first_round_constants: [PackedMontyField31AVX2<PMP>; WIDTH],
    /// Pre-packed round constants for partial rounds (RP-1 entries).
    packed_round_constants: Vec<PackedMontyField31AVX2<PMP>>,
    /// Pre-packed sparse first-row vectors for each partial round (RP entries).
    packed_sparse_first_row: Vec<[PackedMontyField31AVX2<PMP>; WIDTH]>,
    /// Pre-packed rank-1 update vectors for each partial round (RP entries).
    packed_v: Vec<[PackedMontyField31AVX2<PMP>; WIDTH]>,
    _phantom: PhantomData<ILP>,
}

/// The external (full round) layer of Poseidon1 for AVX2-packed MontyField31.
#[derive(Clone)]
    ...
    for Poseidon1InternalLayerMonty31<FP, WIDTH, ILP>
{
    fn new_from_constants(
        ...
        let packed_first_round_constants = internal_constants
            ...
            .map(PackedMontyField31AVX2::from);
        let packed_round_constants = internal_constants
            .round_constants
            .iter()
            .map(|&c| PackedMontyField31AVX2::from(c))
            .collect();
        let packed_sparse_first_row = internal_constants
            .sparse_first_row
            .iter()
            .map(|row| row.map(PackedMontyField31AVX2::from))
            .collect();
...
impl<FP, ILP, const D: u64> PartialRoundLayer<PackedMontyField31AVX2<FP>, 16, D>
    for Poseidon1InternalLayerMonty31<FP, 16, ILP>
where
    FP: FieldParameters + RelativelyPrimePower<D>,
    ILP: PartialRoundBaseParameters<FP, 16> + PartialRoundParametersAVX2<FP, 16>,
{
    fn permute_state(&self, state: &mut [PackedMontyField31AVX2<FP>; 16]) {
        ...
        for r in 0..rounds_p {
            ...
            unsafe {
                ...
                split.s0 = PackedMontyField31AVX2::from_vector(output);
            }
...
impl<FP, ILP, const D: u64> PartialRoundLayer<PackedMontyField31AVX2<FP>, 24, D>
    for Poseidon1InternalLayerMonty31<FP, 24, ILP>
where
    FP: FieldParameters + RelativelyPrimePower<D>,
    ILP: PartialRoundBaseParameters<FP, 24> + PartialRoundParametersAVX2<FP, 24>,
{
...
impl<FP, MU, const D: u64> FullRoundLayer<PackedMontyField31AVX2<FP>, 16, D>
    for Poseidon1ExternalLayerMonty31<FP, MU, 16>
where
    FP: FieldParameters + RelativelyPrimePower<D>,
    MU: MDSUtils,
{
    fn permute_state_initial(&self, state: &mut [PackedMontyField31AVX2<FP>; 16]) {
    ...
    fn permute_state_terminal(&self, state: &mut [PackedMontyField31AVX2<FP>; 16]) {
...
impl<FP, MU, const D: u64> FullRoundLayer<PackedMontyField31AVX2<FP>, 24, D>
    for Poseidon1ExternalLayerMonty31<FP, MU, 24>
where
    FP: FieldParameters + RelativelyPrimePower<D>,
    MU: MDSUtils,
{
    fn permute_state_initial(&self, state: &mut [PackedMontyField31AVX2<FP>; 24]) {
        for round_constants in &self.packed_initial_constants {
            for (s, &rc) in state.iter_mut().zip(round_constants.iter()) {
                add_rc_and_sbox::<FP, D>(s, rc);
            }
    ...
    fn permute_state_terminal(&self, state: &mut [PackedMontyField31AVX2<FP>; 24]) {
        for round_constants in &self.packed_terminal_constants {
            for (s, &rc) in state.iter_mut().zip(round_constants.iter()) {
                add_rc_and_sbox::<FP, D>(s, rc);
            }
