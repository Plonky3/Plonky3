...
impl<MP: FieldParameters + TwoAdicData> RecursiveDft2<MontyField31<MP>> {
    pub fn new(n: usize) -> Self {
        let res = Self {
            twiddles: RefCell::default(),
            inv_twiddles: RefCell::default(),
    ...
    #[inline]
    fn extrapolate_lde_dft(
        &self,
        input: &mut [<MontyField31<MP> as Field>::Packing],
        output: &mut [<MontyField31<MP> as Field>::Packing],
        shift: MontyField31<MP>,
        ncols: usize,
    ) {
        if ncols > 1 {
            ...
            let mut shifts = shift
                .shifted_powers(inv_len)
                .take(ncols)
                .collect::<Vec<_>>();
            reverse_slice_index_bits(&mut shifts);

            input
                .par_chunks_exact_mut(ncols)
                .zip(output.par_chunks_exact_mut(2 * ncols))
                .for_each(|(input_vec, output_vec)| {
                    MontyField31::extrapolate_fft(
                        input_vec,
                        output_vec,
                        twiddles,
                        inv_twiddles,
...
///
impl<MP: MontyParameters + FieldParameters + TwoAdicData> TwoAdicSubgroupDft<MontyField31<MP>>
    for RecursiveDft2<MontyField31<MP>>
{
    ...
    fn coset_lde_batch(
        ...
        if nrows == 1 {
            let dupd_rows = core::iter::repeat(mat.values)
                ...
                .flatten()
                .collect();
            return RowMajorMatrix::new(dupd_rows, ncols);
        }
        ...
        debug_span!("inverse dft batch", n_dfts = ncols, fft_len = nrows)
            .in_scope(|| Self::extrapolate_lde_dft(self, coeffs, packed_padded, shift, nrows));
