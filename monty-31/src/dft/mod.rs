...
pub struct RecursiveDft<F> {
    /// Forward and inverse twiddle tables stored under one lock.
...
impl<MP: FieldParameters + TwoAdicData> RecursiveDft<MontyField31<MP>> {
    ...
    fn update_twiddles(&self, fft_len: usize) {
        ...
        if (current_len + 1) < need {
            // Append only the portion needed in case another thread extended the cache.
            let extend_from = current_len.saturating_sub(have_minus_one);

            let mut twiddles = cache.twiddles.to_vec();
            twiddles.extend_from_slice(&missing_twiddles[extend_from..]);

            let mut inv_twiddles = cache.inv_twiddles.to_vec();
            inv_twiddles.extend_from_slice(&missing_inv_twiddles[extend_from..]);

            cache.twiddles = twiddles.into();
            cache.inv_twiddles = inv_twiddles.into();
        }
    }
