...
//! - The ratio compares the two protocol pipelines, not a pure wrapper cost:
//!   hiding WHIR omits the plain commitment OOD and final-fold phases.
...
const OCTIC_ROUND_LOG_INV_RATE: usize = 6;
const PROOF_REPORT_ENV: &str = "P3_WHIR_PROOF_REPORT";

fn proof_reports_enabled() -> bool {
    std::env::var_os(PROOF_REPORT_ENV).is_some()
}
...
fn octic_no_pow_protocol_params(num_variables: usize) -> ProtocolParameters {
    let folding_factor = FoldingFactor::ConstantFromSecondRound(8, 6);
    let (num_rounds, _) = folding_factor
        .compute_number_of_rounds(num_variables)
        .expect("benchmark folding schedule is valid");
    ProtocolParameters {
        security_level: 128,
        pow_bits: 0,
        // Hold every intermediate codeword at one rate. Deriving the length
        // keeps this fixture valid if OCTIC_OPEN_SIZES gains another round.
        // Rate 2^-6 leaves enough terminal slack for #1995's occupancy-aware
        // hiding budget at every size in OCTIC_OPEN_SIZES.
        round_log_inv_rates: vec![OCTIC_ROUND_LOG_INV_RATE; num_rounds],
        folding_factor,
...
fn octic_no_pow_plain_config(num_variables: usize) -> WhirConfig<OcticEF, F, Challenger> {
    WhirConfig::new_with_initial_claims(
        num_variables,
        octic_no_pow_protocol_params(num_variables),
        1,
    )
    .unwrap()
}
...
impl OcticPlainFixture {
    ...
    fn build_proof(&self) -> (Commitment, OcticPlainProof) {
        ...
        let (commitment, data) = self
            .pcs
            .commit(self.witness.clone(), &mut challenger)
            .unwrap();
        let proof = self
            .pcs
...
impl OcticZkFixture {
    ...
    fn build_proof(&self) -> (Commitment, OcticZkProof) {
        ...
        let proof = self
            .pcs
            .open(data, self.points.clone(), &mut challenger)
            .unwrap();
        (commitment, proof)
    }
}
...
/// Set `P3_WHIR_PROOF_REPORT` to opt in.
...
/// This group uses 128-bit Johnson soundness, start rate 2^-1, round rate
/// 2^-6, folding 8/6, and no PoW. Zero grinding is paid for with extra Merkle
/// queries, so this is deliberately a query-heavy operating point.
fn bench_octic_no_pow(c: &mut Criterion) {
    {
        ...
        for num_variables in OCTIC_OPEN_SIZES {
            ...
            let label = format!(
                "n{num_variables}_ff8_6_round_rate{OCTIC_ROUND_LOG_INV_RATE}_ell3_mask_rate3"
            );

            group.bench_function(BenchmarkId::new("plain", &label), |b| {
                b.iter_batched(
                    || (plain.witness.clone(), plain.challenger()),
                    |(witness, mut challenger)| plain.pcs.commit(witness, &mut challenger).unwrap(),
                    BatchSize::SmallInput,
                );
            });
            group.bench_function(BenchmarkId::new("zk", &label), |b| {
                b.iter_batched(
                    ...
                    |(witness, mut challenger)| zk.pcs.commit(witness, &mut challenger).unwrap(),
                    BatchSize::SmallInput,
                );
            });
        }
            ...
            group.bench_function(BenchmarkId::new("plain", &label), |b| {
                b.iter_batched(
                    || {
                        ...
                        let (_, data) = plain
                            .pcs
                            .commit(plain.witness.clone(), &mut challenger)
                            .unwrap();
                        (
                            challenger,
                            data,
                    ...
                    |(mut challenger, data, protocol, points)| {
                        plain
                            .pcs
                            .open_at(data, &protocol, &points, &mut challenger)
                            .unwrap()
                    },
            ...
            group.bench_function(BenchmarkId::new("zk", &label), |b| {
                b.iter_batched(
                    || {
                        ...
                        let (_, data) = zk.pcs.commit(zk.witness.clone(), &mut challenger).unwrap();
                        (challenger, data, zk.points.clone())
                    },
                    |(mut challenger, data, points)| {
                        zk.pcs.open(data, points, &mut challenger).unwrap()
                    },
                    BatchSize::PerIteration,
                );
            });
            ...
            group.bench_function(BenchmarkId::new("plain", &label), |b| {
                b.iter_batched(
                    ...
                    |(mut challenger, protocol, points)| {
                        // PrescribedPointPcs::verify_at expects the caller to bind
                        // the commitment; the ZK MultilinearPcs::verify path binds
                        // it internally, so both timed legs pay for one binding.
                        plain
                            .pcs
                            .observe_commitment(&plain_commitment, &mut challenger);
                        plain
                            .pcs
                            .verify_at(
                                &plain_commitment,
...
/// and leaf-value counts expose the protocol shape independently. Set
/// `P3_WHIR_PROOF_REPORT` to opt in.
fn report_octic_proof_shape(_c: &mut Criterion) {
    ...
    eprintln!();
    eprintln!(
        "WHIR ZK overhead report (128-bit, no PoW, octic extension, ff=8/6, round rate=2^-{OCTIC_ROUND_LOG_INV_RATE})"
    );
    eprintln!(
