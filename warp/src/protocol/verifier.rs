...
/// Transcript-derived data needed to feed WARP's two sumcheck verifier AIRs.
#[derive(Clone, Debug)]
pub struct WarpStepSumcheckAirWitness<EF> {
    /// Initial claim for the §6.3 twin-constraint sumcheck.
    pub twin_initial_claim: EF,
    /// Fiat-Shamir challenges sampled during the twin-constraint sumcheck.
    pub twin_round_challenges: Vec<EF>,
    /// Final folded claim returned by the twin-constraint sumcheck verifier.
    pub twin_final_claim: EF,
    /// Degree expected for each twin-constraint round polynomial.
    pub twin_degree: usize,
    /// Initial claim for the §8.2 multilinear-batching sumcheck.
    pub batching_initial_claim: EF,
    /// Fiat-Shamir challenges sampled during the multilinear-batching sumcheck.
    pub batching_round_challenges: Vec<EF>,
    /// Final folded claim returned by the multilinear-batching sumcheck verifier.
    pub batching_final_claim: EF,
    /// Degree expected for each multilinear-batching round polynomial.
    ...
    Pesat: BundledPesat<F, EF>,
{
    ...
    /// Verify one externally committed WARP step and return the transcript
    /// data needed by the sumcheck verifier AIRs.
    ///
    /// This first runs the ordinary native verifier, including external PCS
    /// openings. It then replays the Fiat-Shamir transcript from the same
    /// starting state to expose the exact sumcheck challenges and initial/final
    /// claims that an outer SWIRL proof must bind.
    #[instrument(skip_all, name = "warp::verify_external_sumcheck_air_witness")]
    #[allow(clippy::too_many_arguments)]
    pub fn verify_with_external_committed_sumcheck_witness<Challenger, FreshVerifier>(
        &self,
        ...
        new_instance: &AccumulatorInstance<EF, MT::Commitment>,
        proof: &WarpProofExternal<F, EF, MT::Commitment, FreshVerifier::Proof, MT::Proof>,
    ) -> Result<WarpStepSumcheckAirWitness<EF>, VerifierError>
    where
        ...
        FreshVerifier: ExternalCodewordOpeningVerifier<F, Challenger>,
    {
        let mut check_challenger = base_challenger.clone();
        self.verify_with_external_committed(
            &mut check_challenger,
            fresh_verifier,
            fresh_commitments,
            prior_instances,
            new_instance,
        ...
        for c in fresh_commitments {
            fresh_verifier.observe_commitment(&mut challenger, c);
        }
        for &mu_i in &proof.mu_fresh {
            challenger.observe_algebra_element(mu_i);
        }
        ...
        let tau: Vec<EF> = (0..log_l)
            ...
            .collect();
        let tau_eq = Poly::<EF>::new_from_point(&tau, EF::ONE);
        let twin_initial_claim: EF = (0..l)
            .map(|i| tau_eq.as_slice()[i] * (all_mus[i] + omega * all_etas[i]))
            .sum();

        let twin_degree = 1 + (log_n + 1).max(log_m + shape.max_degree);
        let (gamma_pt, twin_final_claim) = verify_sumcheck::<F, EF, _>(
            &proof.twin_constraint_sumcheck,
            &mut challenger,
            twin_initial_claim,
        ...
        let shift_indices =
            sample_indices::<F, _>(&mut challenger, log_n, self.params.num_shift_queries);
        for (k_idx, &x_k) in shift_indices.iter().enumerate() {
            let mut nu_sk = EF::ZERO;
            for i in 0..l1 {
                nu_sk += gamma_eq.as_slice()[i] * EF::from(proof.fresh_shift_answers[k_idx][i]);
        ...
        let r = self.params.r();
        let xi_eq: Vec<EF> = xi_eq_full.as_slice()[..r].to_vec();
        let batching_initial_claim: EF = (0..r).map(|j| xi_eq[j] * nus[j]).sum();

        let batching_degree = 2;
        let (alpha_pt, batching_final_claim) = verify_sumcheck::<F, EF, _>(
            &proof.batching_sumcheck,
            &mut challenger,
            batching_initial_claim,
            log_n,
            batching_degree,
            "multilinear-batching",
        )?;
        ...
        Ok(WarpStepSumcheckAirWitness {
            twin_initial_claim,
            twin_round_challenges: gamma,
            twin_final_claim,
            twin_degree,
            batching_initial_claim,
            batching_round_challenges: alpha_slice.to_vec(),
            batching_final_claim,
            batching_degree,
        })
    }

    /// Verify one externally committed WARP step with an external accumulator
    /// backend and return the transcript data needed by the sumcheck AIRs.
    #[instrument(
        skip_all,
        name = "warp::verify_external_accumulator_sumcheck_air_witness"
    )]
    #[allow(clippy::too_many_arguments)]
    pub fn verify_with_external_committed_accumulator_sumcheck_witness<
        Challenger,
        FreshVerifier,
        AccBackend,
    >(
        ...
        AccBackend: AccumulatorCommitmentBackend<F, EF, Challenger>,
    {
        ...
        for inst in prior_instances {
            acc_backend.observe_commitment(&mut challenger, &inst.rt);
            for &a in &inst.alpha {
                challenger.observe_algebra_element(a);
            }
            challenger.observe_algebra_element(inst.mu);
        ...
        acc_backend.observe_commitment(&mut challenger, &new_instance.rt);
        challenger.observe_algebra_element(proof.nu_0);
        challenger.observe_algebra_element(proof.eta);
