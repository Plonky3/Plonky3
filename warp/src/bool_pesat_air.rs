//! AIR for the final WARP PESAT check of a Boolean trace.
//!
//! This is the arithmetic core for the benchmark relation currently used by
//! `p3-warp`: one Boolean constraint per witness cell,
//!
//! ```text
//!     p_i(w) = w_i * (w_i - 1)
//! ```
//!
//! and the bundled PESAT claim
//!
//! ```text
//!     Pb(beta, w) = sum_i eq(beta, binary(i)) * w_i * (w_i - 1) = eta.
//! ```
//!
//! It intentionally does not bind `w` to any codeword commitment. Compose it
//! with an encoding/commitment verifier before treating the finalizer as
//! complete.
...
/// AIR checking the bundled Boolean PESAT value of one extension witness.
#[derive(Clone, Debug)]
pub struct BinomialBoolPesatAir<F, const D: usize> {
    log_values_len: usize,
    extension_w: F,
}

impl<F, const D: usize> BinomialBoolPesatAir<F, D>
where
    F: Copy,
{
    ...
    pub fn new(log_values_len: usize, extension_w: F) -> Self {
        ...
        assert!(
            log_values_len > 0,
            "Boolean PESAT AIR needs at least two values",
        );
        Self {
            log_values_len,
            extension_w,
...
impl<F, const D: usize> BaseAir<F> for BinomialBoolPesatAir<F, D>
where
    F: Copy + Sync,
{
    fn width(&self) -> usize {
...
#[cfg(feature = "stark-backend")]
impl<F, const D: usize> PartitionedBaseAir<F> for BinomialBoolPesatAir<F, D> where F: Copy + Sync {}

impl<AB, F, const D: usize> Air<AB> for BinomialBoolPesatAir<F, D>
where
    AB: AirBuilder<F = F>,
    F: Field + PrimeCharacteristicRing,
{
    fn eval(&self, builder: &mut AB) {
        ...
        let mut prefix = read_ext::<AB, D>(&local, self.prefix_offset());
        builder.assert_zeros(ext_sub::<AB, D>(prefix.clone(), ext_one::<AB, D>()));
        for (bit_idx, bit) in bits.iter().enumerate() {
            let beta_limb = read_public_ext::<AB, D>(&public_values, bit_idx * D);
            let factor = ext_add::<AB, D>(
                ext_scalar_mul::<AB, D>(
                    ext_sub::<AB, D>(ext_one::<AB, D>(), beta_limb.clone()),
                    AB::Expr::ONE - bit.clone(),
                ),
                ext_scalar_mul::<AB, D>(beta_limb, bit.clone()),
            );
            let next_prefix =
        ...
        let value_minus_one = ext_sub::<AB, D>(value.clone(), ext_one::<AB, D>());
        let bool_constraint = ext_mul::<AB, F, D>(value, value_minus_one, self.extension_w);
        let weighted = ext_mul::<AB, F, D>(bool_constraint, prefix, self.extension_w);
        builder.assert_zeros(ext_sub::<AB, D>(
            acc_out.clone(),
            ext_add::<AB, D>(acc_in, weighted),
        ));

        let public_eta = read_public_ext::<AB, D>(&public_values, self.log_values_len * D);
        builder
            .when_last_row()
            .assert_zeros(ext_sub::<AB, D>(acc_out, public_eta));
    }
}

/// Build the AIR object used for one Boolean PESAT evaluation proof.
#[cfg(feature = "stark-backend")]
pub fn binomial_bool_pesat_air<F, SC, const D: usize>(log_values_len: usize) -> AirRef<SC>
where
    F: Field + BinomiallyExtendable<D> + Sync + 'static,
    SC: StarkProtocolConfig<F = F>,
{
    alloc::sync::Arc::new(BinomialBoolPesatAir::<F, D>::for_binomial_extension(
        log_values_len,
    )) as AirRef<SC>
}

/// Build a trace and public values for one Boolean PESAT evaluation.
pub fn binomial_bool_pesat_air_trace<F, const D: usize>(
    ...
    F: Field + BinomiallyExtendable<D> + PrimeCharacteristicRing,
{
    ...
    let air = BinomialBoolPesatAir::<F, D>::for_binomial_extension(log_values_len);
    let width = air.width();
    ...
    for (index, &value) in values.iter().enumerate() {
        ...
        let constraint = value * (value - BinomialExtensionField::<F, D>::ONE);
        let acc_out = acc + constraint * prefix;
...
/// Build a `stark-backend` proving context for one Boolean PESAT AIR.
#[cfg(feature = "stark-backend")]
pub fn binomial_bool_pesat_air_context<SC, const D: usize>(
...
{
    let (trace, public_values, eta) = binomial_bool_pesat_air_trace::<SC::F, D>(values, point);
    (
...
/// Build a `stark-backend` proving context for one Boolean PESAT AIR.
#[cfg(feature = "stark-backend")]
pub fn binomial_bool_pesat_proving_context<SC, const D: usize>(
    air_id: usize,
    values: &[BinomialExtensionField<SC::F, D>],
    point: &[BinomialExtensionField<SC::F, D>],
) -> (
...
where
    ...
    SC::F: Field + BinomiallyExtendable<D> + PrimeCharacteristicRing,
{
    let (ctx, eta) = binomial_bool_pesat_air_context::<SC, D>(values, point);
    (ProvingContext::new(alloc::vec![(air_id, ctx)]), eta)
}
...
mod tests {
    ...
    fn honest_trace() -> (BinomialBoolPesatAir<F, 4>, RowMajorMatrix<F>, Vec<F>) {
        let values = (0..8)
        ...
        let air = BinomialBoolPesatAir::<F, 4>::for_binomial_extension(3);
        let (trace, public_values, _claim) = binomial_bool_pesat_air_trace::<F, 4>(&values, &point);
        (air, trace, public_values)
    }

    #[test]
    fn binomial_bool_pesat_air_accepts_honest_trace() {
    ...
    fn binomial_bool_pesat_air_rejects_bad_value() {
    ...
    #[test]
    #[should_panic(expected = "constraints not satisfied")]
    fn binomial_bool_pesat_air_rejects_bad_public_eta() {
