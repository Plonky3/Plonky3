...
fn confuse_compiler(x: uint32x4_t) -> uint32x4_t {
    ...
    unsafe {
        ...
        if transmute::<core::arch::aarch64::uint32x4_t, [u32; 4]>(x) != transmute::<core::arch::aarch64::uint32x4_t, [u32; 4]>(y) {
...
mod tests {
    ...
    fn packed_from_canonical(vals: [u32; WIDTH]) -> P {
        PackedKoalaBearNeon(array_from_canonical(vals))
    }
    ...
    fn test_add_vs_scalar() {
        ...
        let vec0 = PackedKoalaBearNeon(arr0);
        let vec1 = PackedKoalaBearNeon(arr1);
