...
use p3_poseidon2::{
    ExternalLayer, InternalLayer, Poseidon2, Poseidon2AVX2, Poseidon2AVX2Methods,
...
fn _poseidon2<F, MdsLight, Diffusion, Constants, const WIDTH: usize, const D: u64>(
    c: &mut Criterion,
    rounds_f: usize,
    rounds_p: usize,
) where
    ...
    Constants: Poseidon2PackedTypesAndConstants<F, WIDTH>,
    MdsLight: ExternalLayer<F, Constants, WIDTH, D> + Default,
    Diffusion:
        Default + InternalLayer<F, Constants, WIDTH, D, InternalState = MdsLight::InternalState>,
{
    let mut rng = thread_rng();
    let external_linear_layer = MdsLight::default();
    let internal_linear_layer = Diffusion::default();

    let poseidon = Poseidon2::<F, MdsLight, Diffusion, Constants, WIDTH, D>::new_from_rng(
        rounds_f,
...
fn _poseidon2_p64<F, MdsLight, Diffusion, Constants, const WIDTH: usize, const D: u64>(
    c: &mut Criterion,
) where
    F: PrimeField64,
    Standard: Distribution<F>,
        ...
        Default + InternalLayer<F, Constants, WIDTH, D, InternalState = MdsLight::InternalState>,
{
    ...
    let poseidon = Poseidon2::<F, MdsLight, Diffusion, Constants, WIDTH, D>::new_from_rng_128(
        external_linear_layer,
        internal_linear_layer,
        &mut rng,
    );
...
// For fields implementing PrimeField64 we should benchmark using the optimal round constants.
fn poseidon2_p64_pf<F, PF, MdsLight, Diffusion, Constants, const WIDTH: usize, const D: u64>(
    c: &mut Criterion,
) where
    F: PrimeField64,
    ...
    Constants: Poseidon2PackedTypesAndConstants<F, WIDTH>,
    MdsLight:
        ExternalLayer<F, Constants, WIDTH, D> + ExternalLayer<PF, Constants, WIDTH, D> + Default,
    Diffusion: Default
        + InternalLayer<
            PF,
            Constants,
            WIDTH,
            D,
            InternalState = <MdsLight as ExternalLayer<PF, Constants, WIDTH, D>>::InternalState,
        >,
{
    let mut rng = thread_rng();
    let external_linear_layer = MdsLight::default();
