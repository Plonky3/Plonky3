...
type CommitmentWithClaims<Val, Challenge, Com> = (
    Com,
    Vec<(CircleDomain<Val>, Vec<(Challenge, Vec<Challenge>)>)>,
);
        ...
        FieldChallenger<Val> + GrindingChallenger<Witness = Val> + CanObserve<FriMmcs::Commitment>,
{
    ...
    let claims = rounds.iter().flat_map(|(_, mats)| {
        mats.iter().flat_map(|(_, points_and_values)| {
            points_and_values
                .iter()
                .map(|(zeta, values)| (Point::from_projective_line(*zeta), values.as_slice()))
        })
    });
        ...
        FieldChallenger<Val> + GrindingChallenger<Witness = Val> + CanObserve<FriMmcs::Commitment>,
{
    ...
    fn open(
        ...
        let alpha = transcript.batch_phase(izip!(&rounds, &values).flat_map(
            |((_, points_for_mats), values_for_mats)| {
            ...
            .map(|(_, mats)| {
                mats.iter()
                    .map(|(_, points_and_values)| {
                        points_and_values.iter().map(|(_, v)| v.len()).collect()
                    })
                    .collect()
            })
            .collect();
