mod coset {
    ...
    fn test_shrink_contained() {
        ...
        let coset = TwoAdicMultiplicativeCoset::<GL>::new(shift, 8).unwrap();
    ...
    fn test_generator_exp() {
        let coset = TwoAdicMultiplicativeCoset::new(BB::ONE, 10).unwrap();
    ...
    fn test_element_wrap_around() {
        let coset = TwoAdicMultiplicativeCoset::new(BB::ONE, 3).unwrap();
    ...
    fn test_element() {
        ...
        let shift: GL = rng.random();
        let coset = TwoAdicMultiplicativeCoset::new(shift, GL::TWO_ADICITY).unwrap();
